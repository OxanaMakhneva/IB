...
#pc_path = "C:\\BD\\from_lesha.xlsx"
bd_path = "C:\\Users\OkCu\Downloads\\vullist.xlsx"
pc_path = "F:\\test\\pc_2.xlsx"

#Загрузка данных по уязвимостям
logger.info("Чтение данных из файла с уязвимостями ...")
try:
  bd_soft = pd.read_excel(bd_path)
except:
  path_file = input("Введите путь до файла с данными по уязвимостям: ")
  bd_soft = pd.read_excel(path_file)
...
# #Загрузка данных по установленному ПО
logger.info("Чтение данных из файла с ПО ...")
try:
    pc_soft = pd.read_excel(pc_path)
    pc_soft.columns = ['name', 'version', 'developer', 'numb_dev', 'comment', 'ts', 'tel', 'update', 'update_rule', 'install']
    #pc_soft.columns = ['name', 'version']
    ...
    pc_soft = pd.read_excel(path_file)
