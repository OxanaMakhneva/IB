...
[(0, 50), (1, 150), (2, 500), (3, 600),])
#Заголовки, которые будут отображаться вместо служебных в основной таблице
HEADERS = (
"ID", "Служба, номер", "Служба, название", "ФИО руководителя",)

QUERY_STR = {}
QUERY_STR['SQLLITE'] = """ SELECT
                 Servises.id,  Servises.numb,
                 Servises.name, Peoples.fio
                 FROM Servises
                 LEFT JOIN Peoples ON Peoples.id = Servises.boss_id
                 WHERE Servises.id > 0
            """
...
DATES = ()
TABLE_ATTR = TabTableAttr("Службы", "службу с ID: ", "servises", "id", "id", 0)
#Дополнительное окно
#Словарь для управления взаимодействием между связанными таблицами (каждая строчка соответствует таблице вкладки)
RELS_DICT = {
"servises":  RelField(type = "OneTab-NoBag", table_tab = "servises", pkey_tab = "id", pkey_tab_idx = 0,  table_bag = "servises", pkey_bag = "id", pkey_bag_idx = 0),
}
#Список для хранения основных данных по таблмцам вкладок
TABS_LIST = (
TabModels(models = {}, relfield = RELS_DICT["servises"], table_name = "servises", tab_name = "Службы", map_list = mapper_lists["servises"], permissions = ("r", "w"), filter_str = "", filter_id = []),
)
#Словарь для хранения моделей данных для общих таблиц
PROM_DICT= {}
DOP_TABLE_WIDTHS = dict([(0, 50), (1, 200), (2, 500)])
