...
DATES = ()
TABLE_ATTR = TabTableAttr("Типы ОРД", "тип с ID: ", "otypes", "id", "id", 0)
#Дополнительное окно
#Словарь для управления взаимодействием между связанными таблицами (каждая строчка соответствует таблице вкладки)
RELS_DICT = {
"otypes":        RelField(type = "OneTab-NoBag", table_tab = "otypes", pkey_tab = "id", pkey_tab_idx = 0,  table_bag = "otypes", pkey_bag = "id", pkey_bag_idx = 0),
}
#Список для хранения основных данных по таблмцам вкладок
TABS_LIST = (
TabModels(models = {}, relfield = RELS_DICT["otypes"], table_name = "otypes", tab_name = "Типы ОРД", map_list = mapper_lists["otypes"], permissions = ("r", "w", "s"), filter_str = "", filter_id = []),
)
