...
DATES = ()
TABLE_ATTR = TabTableAttr("Типы СЗИ", "тип с ID: ", "stypes", "id", "id", 0)
#Дополнительное окно
#Словарь для управления взаимодействием между связанными таблицами (каждая строчка соответствует таблице вкладки)
RELS_DICT = {
"stypes": RelField(type = "OneTab-NoBag", table_tab = "stypes", pkey_tab = "id", pkey_tab_idx = 0,  table_bag = "stypes", pkey_bag = "id", pkey_bag_idx = 0),
}
#Список для хранения основных данных по таблмцам вкладок
TABS_LIST = (
TabModels(models = {}, relfield = RELS_DICT["stypes"], table_name = "stypes", tab_name = "Типы СЗИ", map_list = mapper_lists["stypes"], permissions = ("r", "w", "s"), filter_str = "", filter_id = []),
)
