...
HEADERS = (
"ID", "Тип ответсвтенного", "Описание")

QUERY_STR = {}
QUERY_STR['SQLLITE'] = """ SELECT
                 RTypes.id, RTypes.type, RTypes.kom
                 FROM RTypes
                 WHERE RTypes.id > 0
            """
...
DATES = ()
TABLE_ATTR = TabTableAttr("Типы ответсвенных", "тип с ID: ", "rtypes", "id", "id", 0)
#Дополнительное окно
#Словарь для управления взаимодействием между связанными таблицами (каждая строчка соответствует таблице вкладки)
RELS_DICT = {
"rtypes": RelField(type = "OneTab-NoBag", table_tab = "rtypes", pkey_tab = "id", pkey_tab_idx = 0,  table_bag = "rtypes", pkey_bag = "id", pkey_bag_idx = 0),
}
#Список для хранения основных данных по таблмцам вкладок
TABS_LIST = (
TabModels(models = {}, relfield = RELS_DICT["rtypes"], table_name = "rtypes", tab_name = "Типы ответственных", map_list = mapper_lists["rtypes"], permissions = ("r", "w", "s"), filter_str = "", filter_id = []),
)
