...
DATES = ()
TABLE_ATTR = TabTableAttr("Типы документов", "тип с ID: ", "dtypes", "id", "id", 0)
#Дополнительное окно
#Словарь для управления взаимодействием между связанными таблицами (каждая строчка соответствует таблице вкладки)
RELS_DICT = {
"dtypes":        RelField(type = "OneTab-NoBag", table_tab = "dtypes", pkey_tab = "id", pkey_tab_idx = 0,  table_bag = "dtypes", pkey_bag = "id", pkey_bag_idx = 0),
}
#Список для хранения основных данных по таблмцам вкладок
TABS_LIST = (
TabModels(models = {}, relfield = RELS_DICT["dtypes"], table_name = "dtypes", tab_name = "Типы документов", map_list = mapper_lists["dtypes"], permissions = ("r", "w", "s"), filter_str = "", filter_id = []),
)
