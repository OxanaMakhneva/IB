...
QUERY_STR['SQLLITE'] = """ SELECT
                 SElements.id, SElements.name
                 FROM SElements
                 WHERE SElements.id > 0
            """
...
LABLES = (
Lables("Панель для уточнения поискового запроса (условия применяются одновременно)", 11, 0, 0, 1, 2),
Lables("Элемент...", 9, 2, 0, 1, 1),
        )
#Именованный кортеж для поля фильтрации:
EDITS = (Edits(1, "Элемент ...", 2, 1, 1, 1, dict = {}),)
#Именованный кортеж для выпадающих списков поля фильтрации:
COMBOS = ()
#Именованный кортеж для ввода дат в поле фильтрации:
DATES = ()
TABLE_ATTR = TabTableAttr("Элементы СЗИ", "элемент с ID: ", "selements", "id", "id", 0)
#Дополнительное окно
#Словарь для управления взаимодействием между связанными таблицами (каждая строчка соответствует таблице вкладки)
RELS_DICT = {
"selements": RelField(type = "OneTab-NoBag", table_tab = "selements", pkey_tab = "id", pkey_tab_idx = 0,  table_bag = "selements", pkey_bag = "id", pkey_bag_idx = 0),
}
#Список для хранения основных данных по таблмцам вкладок
TABS_LIST = (
TabModels(models = {}, relfield = RELS_DICT["selements"], table_name = "selements", tab_name = "Элементы СЗИ", map_list = mapper_lists["selements"], permissions = ("r", "w"), filter_str = "", filter_id = []),
)
